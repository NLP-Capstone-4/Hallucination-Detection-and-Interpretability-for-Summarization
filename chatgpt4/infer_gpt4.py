...
def one_shot_tags(row, logger, model_name):
    dialogue = row[1]
    summary = row[3]
    curr_model_name = row[-1].strip()

    tags = None
    explanation = None
    if curr_model_name == model_name:
        prompt_1 = """You are very good at conversation token classification. Given a conversation and its summary, the task is to do token-level classification based on whether it is hallucinated or not. Use the following tag classes to label each token of the summary.
        ...
        Similarly, for the next dialogue, generate tags for the summary. Think step by step to explain it.
        Conversation- 
        """
        ...
        try:
            ...
            messages=[
                {
                "role": "user",
                "content": prompt_1 + str(row[1]) + prompt_2 + str(row[3]) + prompt_3
                }
            ],
            temperature=0.001,
            max_tokens=256,
            ...
            if len(choices) > 0:
                data = choices[0]['message']['content']
                (summary, tags, explanation) = extract_summary_tags(data) # summary will be always None bcz its not generated
                logger.info("Successfully infered conv_id:" + str(row[0]))
            else:
                logger.error("Error - Conversaition ID - " + str(row[0]) + response.status_code)
        except Exception as e:
