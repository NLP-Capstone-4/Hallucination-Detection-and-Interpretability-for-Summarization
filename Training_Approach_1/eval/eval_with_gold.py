...
def tokenize(source_tokens):
    ...
    return tokenized_inputs, source_tokens.index(SEP1_TOKEN), source_tokens.index(SEP2_TOKEN)
...
'''
def predict(tokenized_inputs, sep1_idx, sep2_idx):
    ...
    summary_prediction_labels = source_prediction_labels[sep2_idx+1:] #slice prediction for summary segment from source
...
'''
def predict_dialog_summary(dialog, gold_summary, summary):
    
    # Generate Source = DIALOG + <SEP1> + GOLD SUMMARY + <SEP2> + SUMMARY + <EOS>
    source = curate_text(dialog) + f' {SEP1_TOKEN} ' + curate_text(gold_summary) + f' {SEP2_TOKEN} ' + curate_text(summary) + f' {EOS_TOKEN} '
    source_tokens = source.split()  # whilte space tokenize source
    
    tokenized_inputs, sep1_idx, sep2_idx = tokenize(source_tokens) #encode tokens using model's tokenizer
    
    #get source and summary prediction using the encoded tokenized data
    source_prediction_labels, summary_prediction_labels = predict(tokenized_inputs, sep1_idx, sep2_idx)
    
    
    dialog = ' '.join(source_tokens[0:sep1_idx])
    gold_summary = ' '.join(source_tokens[sep1_idx+1:sep2_idx])
    generated_summary_tokens = source_tokens[sep2_idx+1:]
...
def test_predict_dialog_summary():
    ...
    gold_summary = "Mike took his car into garage today . Ernest is relieved as someone had just crashed into a red Honda which looks like Mike's ."
    summary= "Mike's car has been damaged beyond repair after being hit by another car ."
    
    predict_dialog_summary(dialog, gold_summary, summary)  # predict source
...
def test_predict_dataset(ds, filename):
    ...
    for i in range(len(ds['source'])):
        ...
        gold_summary_tokens = source_tokens[sep1_idx+1:sep2_idx]
        gold_summary = ' '.join(gold_summary_tokens)
        
        generated_summary_tokens = source_tokens[sep2_idx+1:]
        generated_summary = ' '.join(generated_summary_tokens)
        
        gold_tag_ids = gold_tags_tokens[sep2_idx+1:]
...
tokenizer = AutoTokenizer.from_pretrained('Deojoandco/capstone_hal_with_gold', add_prefix_space=True)
model = AutoModelForTokenClassification.from_pretrained('Deojoandco/capstone_hal_with_gold').to(device)
...
test_predict_dataset(ds['train'], 'prediction_output_withgold_train.csv')
print('Predicting validation dataset...')
test_predict_dataset(ds['validation'], 'prediction_output_withgold_validation.csv')
print('Predicting test dataset...')
test_predict_dataset(ds['test'], 'prediction_output_withgold_test.csv')